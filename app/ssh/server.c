...
int ssh_server_thread()
{
    int ret=0;
    pthread_t tid1;
    ret = pthread_create(&tid1, NULL, ssh_main, NULL);
    if(ret)
    {
        printf("create ssh_server_thread error!\n");
        return -1; 
    }

    printf("create ssh_server_thread successfully!\n");
    return 0;
}
...
int open_pty(int client_fd) 
{
    ...
    if( unlockpt(pty_master) == -1 || (pty_slave = ptsname(pty_master)) == NULL) {
