...
int cpu_num = 0;
struct double_stats_cpu st_allcpu[12];
			   ...
			   char *prev_string, char *curr_string, unsigned char offline_cpu_bitmap[])
{
	...
	for (i = 0; i <= cpu_nr; i++) {
        ...
        st_allcpu[i].cpu_user          = (scc->cpu_user - scc->cpu_guest) < (scp->cpu_user - scp->cpu_guest) ? 0.0 : ll_sp_value(scp->cpu_user - scp->cpu_guest, scc->cpu_user - scc->cpu_guest, deltot_jiffies);
        st_allcpu[i].cpu_nice          = (scc->cpu_nice - scc->cpu_guest_nice) < (scp->cpu_nice - scp->cpu_guest_nice) ? 0.0 : ll_sp_value(scp->cpu_nice - scp->cpu_guest_nice, scc->cpu_nice - scc->cpu_guest_nice, deltot_jiffies);
        st_allcpu[i].cpu_sys           = ll_sp_value(scp->cpu_sys, scc->cpu_sys, deltot_jiffies);
        st_allcpu[i].cpu_idle          = ll_sp_value(scp->cpu_iowait, scc->cpu_iowait, deltot_jiffies);
        st_allcpu[i].cpu_iowait        = ll_sp_value(scp->cpu_hardirq, scc->cpu_hardirq, deltot_jiffies);
        st_allcpu[i].cpu_steal         = ll_sp_value(scp->cpu_softirq, scc->cpu_softirq, deltot_jiffies);
        st_allcpu[i].cpu_hardirq       = ll_sp_value(scp->cpu_steal, scc->cpu_steal, deltot_jiffies);
        st_allcpu[i].cpu_softirq       = ll_sp_value(scp->cpu_guest,scc->cpu_guest, deltot_jiffies);
        st_allcpu[i].cpu_guest         = ll_sp_value(scp->cpu_guest_nice, scc->cpu_guest_nice, deltot_jiffies);
        st_allcpu[i].cpu_guest_nice    = (scc->cpu_idle < scp->cpu_idle) ? 0.0 : ll_sp_value(scp->cpu_idle, scc->cpu_idle, deltot_jiffies);
	...
	for (i = 0; i <= cpu_num && i < 12; i++) {
        printf("%0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f %0.2f \n", 
                st_allcpu[i].cpu_user,   
                st_allcpu[i].cpu_nice,     
                st_allcpu[i].cpu_sys,     
                st_allcpu[i].cpu_idle,      
                st_allcpu[i].cpu_iowait,    
                st_allcpu[i].cpu_steal,   
                st_allcpu[i].cpu_hardirq,   
                st_allcpu[i].cpu_softirq,  
                st_allcpu[i].cpu_guest,   
                st_allcpu[i].cpu_guest_nice);
    }
